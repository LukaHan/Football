...
class ScoreFragment(private val activity: HomeActivity, private val index: Int) : BaseFragment(), SwipeRefreshLayout.OnRefreshListener {

    private var list: ArrayList<LeagueEntity.ViewsBean.JifenbangBean>? = null
    ...
    private fun initData() {
        list = ArrayList<LeagueEntity.ViewsBean.JifenbangBean>()
        mAdapter = ScoreAdapter(getActivity() as BaseActivity, list as ArrayList<LeagueEntity.ViewsBean.JifenbangBean>)
    ...
    private fun setData(result: LeagueEntity) {
        if (index == 0) {
            activity.supportActionBar!!.title = result.tabs.jifenbang

            list!!.addAll(result.views.jifenbang)
        } else {
            activity.supportActionBar!!.title = result.tabs.sheshoubang

            list!!.addAll(result.views.sheshoubang)
        }
